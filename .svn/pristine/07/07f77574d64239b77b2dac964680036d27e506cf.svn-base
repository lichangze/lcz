...
$.jgrid.extend({
	searchGrid : function (p) {
		...
		return this.each(function() {
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				if(p.showQuery) {
					bQ ="<a id='"+fid+"_query' class='fm-button ui-state-default ui-corner-all fm-button-icon-left btn btn-sm btn-primary'><span class='ui-icon ui-icon-comment'></span>Query</a>";
				}
				if(!p.columns.length) {
	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			bS  ="<a id='sData' class='fm-button ui-state-default ui-corner-all btn btn-sm btn-primary'>"+p.bSubmit+"</a>",
			bC  ="<a id='cData' class='fm-button ui-state-default ui-corner-all btn btn-sm btn-primary'>"+p.bCancel+"</a>";
