...
AutoPage={
	...
	getTrRecord:function(startLine,offsetLine){
		var trStr="";
		trStr+="<tr style='background-color: #ccc'>"+$("#tabletitle").html()+"<tr>";
		$("#"+AutoPage.content).find("tbody tr").slice(startLine,offsetLine).each(function(i){
			var bag=$(this).attr("id");
			trStr += "<tr style='background-color: "+bag+"' >"+$(this).html()+"</tr>";
		});
		return trStr;
	}
};
$(function(){
	...
	*　pageNum　页码样式　class
	×　10　每页显示多少条记录
	*/
	AutoPage.init("headerInfo", "tabContent", "footerInfo","pageBreak","pageNum",16);
});
