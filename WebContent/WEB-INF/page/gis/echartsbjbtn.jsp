...
<body>
	...
	<div id="main" style="width: 1910px; height: 1080px;"></div>
...
//点
	...
	$.ajax({
		...
		var convertData = function(databtn) {
		    ...
		    for (var i = 0; i < databtn.length; i++) {
		        ...
		        if (geoCoord) {
		            res.push({
		                name: databtn[i].name,
		                value: geoCoord.concat(databtn[i].value,databtn[i].valueshow,databtn[i].valuein,databtn[i].valueout),
		            });
		        }
		    }
		    return res;
		...
		var schema = [
		    {name: 'date', index: 0},
		    {name: '进站客流', index: 1, text: '进站客流'},
		    {name: '客流量', index: 2, text: '客流量'},
		    {name: '出站客流', index: 3, text: '出站客流'},
    ...
    window.setInterval(function () {
    	$.ajax({
    		...
    		success : function(data) {
    			var data = $.parseJSON(data);
    			geoCoordMap = eval('(' + data.station + ')');
       			refreshData(convertData(eval('(' + data.value + ')'))); 
       			data = null;
    		}
    	});
    },1000*60*1);
